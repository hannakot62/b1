...
import {addFav} from "../store/slices/favsSlice.js";
import modalStyle from "../components/Modal/Modal.module.css";

export default function UnfilledHeartIcon({setActiveModal,setModalChildren, symbol, iconUrl, uuid}) {
    const dispatch = useDispatch()

    const handleClick = () => {
        const handleAdd = () => {
            dispatch(addFav(uuid))
            setActiveModal(false)
        }

        setModalChildren(<>
            <h3>Are you sure you want to add {symbol} to favourites?</h3>
            <img src={iconUrl} alt={symbol}/>
            <div className={modalStyle.buttons}>
                <button className={modalStyle.add} onClick={() => handleAdd()}>add</button>
            ...
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff">
                ...
                <g id="SVGRepo_iconCarrier">
                    <path
                        ...
                        stroke="#ffffff" strokeWidth="2" strokeLinejoin="round"></path>
                </g>
            </svg>
        </div>
    )
